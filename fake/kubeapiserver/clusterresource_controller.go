...
func NewClusterResourceController(restManager *RESTManager, discoveryManager *discovery.DiscoveryManager) *ClusterResourceController {
	...
	err := json.Unmarshal([]byte(pediaClusterTemp), pediaCluster)
	if err != nil {
		klog.Error(err)
	}

	controller.updateClusterResources(pediaCluster)
